...
async def test_guard_check_success(config: CoreasonBudgetConfig) -> None:
    ...
    # Should pass
    result = await guard.check("user1", "proj1", 5.0)
...
async def test_guard_charge(config: CoreasonBudgetConfig) -> None:
    ...
    # Mock datetime to check TTL logic if needed, but integration test is better for that.
